        ...
        || SceneID.IsBWPScene(ActiveScene))
            {
                ...
                else if (ActiveScene == SceneID.Home)
                {
                    ...
                    activeLights = activeLights
                        // Directional light brightness changes becuase of ortho micast in scene 2 town if i order shadows first. but this doesnt effect scene 3 why? becuase of a light order mismatch?
                      .OrderBy(l => l.Type == LightType.Directional ? 0 : 1)
                      // if this is changed to order by for cast shadows it enables point light shadows on scene 3 but disables foliage shadows and vice versa
                      .ThenBy(l => l.CastsShadows ? 0 : 1)
                      .ThenBy(l => Vector3.DistanceSquared(l.Position, camPos))
                      .ToList();
